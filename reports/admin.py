...
class OrderReportAdmin(admin.ModelAdmin):
    ...
    def changelist_view(self, request, extra_context=None):
        ...
        monthly_stats = (
            ...
            .values("year", "month")  
            .annotate(sum=Sum("transaction__amount"))[:30]  #? here we take last 30 (Surely I mean last in time not last in the list 😒) because here we want last 30 month
        ...
        weekly_stats = (
            ...
            .values("year", "week")
            .annotate(sum=Sum("transaction__amount"))[:30]  #? here we want last 30 week
            #! in my opinion the slicing above has done in a wrong way (I think the true slicing is [-30:])
        )
